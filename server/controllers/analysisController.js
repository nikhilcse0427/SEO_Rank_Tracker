...
import { analyzeSeoDataFallback } from "../services/fallbackSeoService.js";
import { scrapeUrlFetch } from "../services/fetchScraperService.js";
...
export const analyzeUrl = async (req, res) => {
    try {
        ...
        try {
            // Step 1: Scrape the URL (Browserbase, then fetch fallback)
            let scrapeResult = await scrapeUrl(validUrl.href);

            if (!scrapeResult.success) {
                console.warn("Browser scrape failed, using fetch fallback:", scrapeResult.error);
                scrapeResult = await scrapeUrlFetch(validUrl.href);
            }
            ...
            // Step 2: Analyze with Gemini AI (fallback to rule-based if API unavailable)
            let aiResult = await analyzeSeoData(scrapeResult.data);

            if (!aiResult.success) {
                console.warn("Gemini unavailable, using rule-based analysis:", aiResult.error);
                aiResult = analyzeSeoDataFallback(scrapeResult.data);
            }
